...
class RandomRotation:
    ...
    def __init__(self, keys, degrees, rotate_ratio=0.5):
...
@PIPELINES.register_module()
class CutBlur:
    """The LQ image and GT image should have the same size here."""

    def __init__(self, keys=['lq','gt'], cutblur_ratio=0.4, alpha=1.0):
        self.keys = keys
        self.cutblur_ratio = cutblur_ratio
        self.alpha = alpha

    def __call__(self, results):
        cutblur = np.random.random() < self.cutblur_ratio
        ...
        if cutblur:
            ...
            if np.random.random() > 0.5:
                results['gt'][cy:cy+ch, cx:cx+cw,...] = copy.deepcopy(results['lq'][cy:cy+ch, cx:cx+cw,...])
            else:
                results['lq'][cy:cy+ch, cx:cx+cw,...] = copy.deepcopy(results['gt'][cy:cy+ch, cx:cx+cw,...])
        results['cutblur'] = cutblur
        results['cutblur_alpha'] = self.alpha

        return results

    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += (f'(keys={self.keys}, cutblur_ratio={self.cutblur_ratio}, '
                     f'alpha={self.alpha})')
        return repr_str
...
class RGB_Perm:
    ...
    def __init__(self, keys, perm_ratio=0.5):
        self.keys = keys
        self.perm_ratio = perm_ratio

    def __call__(self, results):
        perm = np.random.random() < self.perm_ratio

        if perm:
            for key in self.keys:
                results[key] = results[key][:, np.random.permutation(3)]
    ...
    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += (f'(keys={self.keys}, blend_ratio={self.perm_ratio}')
        return repr_str
...
class Blend:
    ...
    def __init__(self, keys, blend_ratio=0.5, alpha=0.6):
        self.keys = keys
        self.blend_ratio = blend_ratio
        self.alpha = alpha

    def __call__(self, results):
        blend = np.random.random() < self.blend_ratio

        if blend:
            for key in self.keys:
                c = torch.empty((results[key].size(0), 3, 1, 1), device=results[key].device).uniform_(0, 255)
                rimg = c.repeat((1, 1, results[key].size(2), results[key].size(3)))
                v = np.random.uniform(self.alpha, 1)
                results[key] = v * results[key] + (1-v) * rimg
                
        results['blend'] = blend
        results['blend_alpha'] = self.alpha

        return results

    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += (f'(keys={self.keys}, blend_ratio={self.blend_ratio}, '
                     f'alpha={self.alpha})')
        return repr_str
...
class ColorJitter:
    ...
    def __init__(self, keys, to_rgb=False, jitter_ratio=0.5, **kwargs):
        assert keys, 'Keys should not be empty.'
    ...
    def __call__(self, results):
        jitter = np.random.random() < self.jitter_ratio
        if jitter:
            for k in self.keys:
                if self.to_rgb:
